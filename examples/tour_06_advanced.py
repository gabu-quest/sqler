...
def _(Author, Book):
    ...
    Book(title="Carol's Book 1", author=_carol).save()
    Book(title="Carol's Book 2", author=_carol).save()

    _carol_id = _carol._id
    _books_before = Book.query().filter(Book.ref("author").field("_id") == _carol_id).count()
    print(f"Created Carol with {_books_before} books")
    ...
    print(f"Carol's books remaining: {len(_carol_books_after)}")
    return
