...
@app.cell
def _(Counter, StaleVersionError):
    # Create a counter
    ...
    # First increment works
    _view1.value += 1
    _view1.save()
    print(f"After view1 increment: {_view1.value}")

    # Second increment - with rebasing, conflicts may be auto-resolved
    _view2.value += 1
    try:
        _view2.save()
        print(f"After view2 increment: {_view2.value} (rebased!)")
    except StaleVersionError:
        print("view2: Conflict detected, refreshing and retrying...")
        _view2.refresh()
        _view2.value += 1
        _view2.save()
        print(f"After view2 retry: {_view2.value}")
