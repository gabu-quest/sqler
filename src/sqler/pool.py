...
class PooledSQLiteAdapter:
    ...
    def begin_transaction(self) -> None:
        ...
        if self._txn_depth == 0:
            ...
            if hasattr(self._local, "txn_conn") and self._local.txn_conn is not None:
                self._local.txn_conn.execute(f"SAVEPOINT sp_{self._txn_depth}")
        self._txn_depth += 1
    ...
    def end_transaction(self, *, commit: bool = True) -> None:
        ...
        if self._txn_depth == 0:
            ...
            if hasattr(self._local, "txn_conn") and self._local.txn_conn is not None:
                ...
                try:
                    if commit:
                        self._local.txn_conn.execute(f"RELEASE SAVEPOINT {sp}")
                    else:
                        self._local.txn_conn.execute(f"ROLLBACK TO SAVEPOINT {sp}")
                        self._local.txn_conn.execute(f"RELEASE SAVEPOINT {sp}")  # Clean up
